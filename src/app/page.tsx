...
import { FaSearch, FaFire, FaShoppingCart, FaTimes, FaTruck } from 'react-icons/fa';
...
export default function HomePage() {
  ...
  const fetchProductos = async () => {
    try {
      ...
      const productosTransformados = data.map((p: any) => ({
        ...p,
        categoria: p.categoria || 'Sin categoría',
        fotos: Array.isArray(p.fotos) ? p.fotos : [],
        imagen_url: p.imagen_url || (Array.isArray(p.fotos) && p.fotos.length > 0 ? p.fotos[0] : ''),
      })) as Producto[];
  ...
  const getDiscount = (producto: Producto) => {
    if (producto.precio_oferta_usd && producto.precio_venta_usd > producto.precio_oferta_usd) {
      return Math.round(
        ((producto.precio_venta_usd - producto.precio_oferta_usd) / producto.precio_venta_usd) * 100
      );
    }
    return 0;
  };
  ...
  const obtenerImagen = (producto: Producto): string => {
    // Prioridad: imagen_url > fotos[0] > vacío
        ...
        {filteredProductos.length === 0 ? (
          ...
          >
            {filteredProductos.map(producto => {
              ...
              const imagenProducto = obtenerImagen(producto);
